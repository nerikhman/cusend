...
struct my_scheduler_with_on_free_function : my_executor
{
  ...
  __host__ __device__
  friend ns::dot::on_t<Sender,ns::execution::inline_executor> on(Sender&& sender, const my_scheduler_with_on_free_function&)
  {
    ++num_calls_to_customizations;
    return ns::dot::on(std::forward<Sender>(sender), ns::execution::inline_executor{});
  }
};
...
void test(Executor ex)
{
  ...
  // submit on ex
  auto sender = ns::dot::on(std::move(xfrm_arg1), ex);
