...
#include <cusend/dot/transform.hpp>
...
void test_is_typed_sender()
{
  ...
  {
    auto result = dot::transform(just(), [] { return; });
    static_assert(is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = dot::transform(just(), [] { return 13; });
    static_assert(is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = dot::transform(just(13), [](int arg) { return arg; });
    static_assert(is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = dot::transform(just(13,7), [](int arg1, int arg2) { return arg1 + arg2; });
    static_assert(is_typed_sender<decltype(result)>::value, "Error.");
  }
}
...
void test_copyable_continuation()
{
  ...
  dot::transform(just(arg1), [=] (int arg1) { return arg1 + arg2; }).connect(std::move(r)).start();
...
void test_move_only_continuation()
{
  ...
  dot::transform(just(arg1), std::move(continuation)).connect(std::move(r)).start();
...
struct my_sender_with_transform_member_function
{
  ...
  auto transform(Function continuation) &&
    -> decltype(ns::dot::transform(ns::just(arg), continuation))
  {
    return ns::dot::transform(ns::just(arg), continuation);
  }
};
...
void test_sender_with_transform_member_function()
{
  ...
  ns::dot::transform(std::move(s), [=](int arg1) {return arg1 + arg2;}).connect(std::move(r)).start();
...
auto transform(my_sender_with_transform_free_function&& s, Function continuation)
  -> decltype(ns::dot::transform(ns::just(s.arg), continuation))
{
  return ns::dot::transform(ns::just(s.arg), continuation);
}
