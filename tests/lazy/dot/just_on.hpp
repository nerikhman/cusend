...
#include <cusend/lazy/dot/just_on.hpp>
...
{
  {
    auto result = ns::dot::just_on(ex);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = ns::dot::just_on(ex,1);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = ns::dot::just_on(ex,1,2);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    auto result = ns::dot::just_on(ex,1,2,3);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }
}
...
template<class Executor>
__host__ __device__
void test_is_chaining(Executor ex)
{
  {
    auto result = ns::dot::just_on(ex);
...
void test_copyable(Executor ex)
{
  ...
  ns::dot::just_on(ex, expected).connect(std::move(r)).start();
...
void test_move_only(Executor ex)
{
  ...
  ns::dot::just_on(ex, move_only{expected}).connect(std::move(r)).start();
...
void test_variadic(Executor ex)
{
...

  {
    ...
    ns::dot::just_on(ex).connect(std::move(r)).start();
...

  {
    ...
    ns::dot::just_on(ex, expected1).connect(std::move(r)).start();
...

  {
    ...
    ns::dot::just_on(ex, expected1, expected2).connect(std::move(r)).start();
...

  {
    ...
    ns::dot::just_on(ex, expected1, expected2, expected3).connect(std::move(r)).start();
...
struct my_executor_with_just_on_member_function : ns::execution::inline_executor
{
  ...
  auto just_on(Types&&... values) const
    -> decltype(ns::dot::just_on(ns::execution::inline_executor(), std::forward<Types>(values)...))
  {
    return ns::dot::just_on(ns::execution::inline_executor(), std::forward<Types>(values)...);
  }
};
...
void test_just_on()
{
  ...
  test_is_chaining(ns::execution::inline_executor{});
  test_is_chaining(my_executor_with_just_on_member_function{});
  test_is_chaining(my_executor_with_just_on_free_function{});
  test_is_chaining(my_scheduler{});
  test_is_chaining(my_scheduler_with_just_on_member_function{});
  test_is_chaining(my_scheduler_with_just_on_free_function{});
