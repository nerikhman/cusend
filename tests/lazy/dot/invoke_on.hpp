...
#include <cusend/lazy/dot/invoke_on.hpp>
...
void test_is_typed_sender(Executor ex)
{
  {
    // test with 0 args
    auto result = ns::dot::invoke_on(ex, [] __host__ __device__ { return ;});
...

  {
    ...
    auto result = ns::dot::invoke_on(ex, identity, 13);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }

  {
    ...
    auto result = ns::dot::invoke_on(ex, plus, 13, 7);
    static_assert(ns::is_typed_sender<decltype(result)>::value, "Error.");
  }
}
...
void test_invoke_on()
{
  ...
  test_is_chaining(my_executor_with_invoke_on_member_function{});
  test_is_chaining(my_executor_with_invoke_on_free_function{});
  test_is_chaining(my_scheduler{});
  test_is_chaining(my_scheduler_with_invoke_on_member_function{});
  test_is_chaining(my_scheduler_with_invoke_on_free_function{});
